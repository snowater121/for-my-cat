...
body{background:radial-gradient(1100px 640px at 82% -12%,var(--bg1),var(--bg2) 60%);color:var(--text);font-family:"Noto Sans KR","Noto Sans JP",-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif;display:flex;flex-direction:column;height:100vh;overflow:hidden}
...
@media(max-width:820px){
  ...
  aside{max-width:none;min-width:0;border-left:0;border-top:2px solid var(--accent);flex:none;max-height:32vh}
...
/* phones: compact header, keep everything readable */
@media(max-width:600px){
  header{padding:8px 10px;gap:7px}
  .brand{font-size:14px;width:100%;order:1}
  .brand small{display:none}
  .tools{order:2;gap:6px;width:100%;justify-content:flex-start}
  ...
  .settings{width:min(90vw,240px)}
  .mapwrap{height:46vh}
  aside{max-height:30vh;padding:9px}
  .region h3{font-size:12px}
  .chip{font-size:11px;padding:3px 7px}
  #answer{font-size:20px;padding:12px 14px}
  .qbtns .btn{padding:8px 10px}
  .ejucard{max-height:58%;width:calc(100% - 24px);padding:12px 14px;bottom:12px}
  .ejucard li{font-size:14px}
  .ejucard .qname h2{font-size:24px}
  .opt{font-size:14px;padding:9px 11px}
  .startbtn{font-size:16px;padding:16px 24px}
  .countdown{font-size:clamp(40px,16vw,64px)}
  .credit{font-size:6px}
  .overview{width:92px;height:80px;top:8px;right:8px}
  svg#ovmap{height:60px}
  .typinglabel{display:none}
}
/* landscape phones: shrink map so input stays reachable */
@media(max-height:520px) and (orientation:landscape){
  main{flex-direction:row}
  .mapwrap{flex:1.4;height:auto}
  aside{max-height:none;max-width:230px;border-top:0;border-left:2px solid var(--accent)}
...
function vbForTarget(code){
  const b=bbox[code]; const pad=Math.max(b.w,b.h)*0.85+10;
