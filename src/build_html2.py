...
.setlinks{display:flex;gap:14px;padding:8px 2px 2px;border-top:1px dashed var(--line);font-size:12px;font-weight:700}
.setlinks a{color:var(--sub);text-underline-offset:3px;padding:4px 0}
.setlinks a:hover,.setlinks a:focus-visible{color:var(--text)}
  ...
  <div class="tools">
    ...
    <div class="gearwrap">
      ...
      <div class="settings" id="settings">
        ...
        <div class="setlinks"><a href="terms.html">이용약관</a><a href="privacy.html">개인정보처리방침</a></div>
      </div>
    </div>
  </div>
  <div class="stats">
...
function ejuReveal(code,correct){
  ...
  const e=EJU[code];
  hint.innerHTML=(correct?`<b style="color:var(--ok)">✅ 정답! ${e.ko} · ${e.kanji}</b>`:`<b style="color:var(--gold)">정답: ${e.ko} · ${e.kanji}</b>`)+` <span style="font-weight:500">${e.detail}</span>`;
}
